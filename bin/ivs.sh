...
# If VIEWMAT_CONTROL_MODE is not set, set to mouse.
if [[ -z "$VIEWMAT_CONTROL_MODE" ]]; then
...
#
# glutFullScreen() doesn't fill the top row of pixels on IVS, so we'll
# use explicit window size and positioning.
#
# __GL_SYNC_TO_VBLANK is an NVIDIA option that prevents tearing.
...
export VIEWMAT_CONTROL_MODE='${VIEWMAT_CONTROL_MODE}'
